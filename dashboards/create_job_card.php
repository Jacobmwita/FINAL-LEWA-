...
/**
 * Fetches all users with the 'service_advisor' user_type from the database.
 * @param mysqli $conn The database connection object.
 * @return array An array of service advisors with user_id and full_name.
 */
function getServiceAdvisors(mysqli $conn): array {
    $advisors = [];
    $stmt = $conn->prepare("SELECT user_id, full_name FROM users WHERE LOWER(user_type) = 'service_advisor' ORDER BY full_name ASC");
    if ($stmt) {
        $stmt->execute();
        ...
        error_log("Failed to prepare getServiceAdvisors statement: " . $conn->error);
    }
    return $advisors;
}

// Fetch all service advisors for the dropdown
    ...
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
        ...
        try {
            if (isset($_POST['action']) && $_POST['action'] === 'update_job_card') {
                ...
                $service_advisor_id_raw = filter_input(INPUT_POST, 'service_advisor_id', FILTER_SANITIZE_FULL_SPECIAL_CHARS); // New: Get raw SA ID

                $assigned_to = empty($assigned_to_raw) ? NULL : filter_var($assigned_to_raw, FILTER_VALIDATE_INT);
                $service_advisor_id = empty($service_advisor_id_raw) ? NULL : filter_var($service_advisor_id_raw, FILTER_VALIDATE_INT); // New: Validate SA ID
                ...
                if ($job_card_id === false || empty($vehicle_reg) || empty($issue_desc) || empty($status)) {
                    ...
                    // MODIFIED: Added service_advisor_id to the UPDATE query
                    $stmt = $conn->prepare("UPDATE job_cards SET vehicle_registration = ?, issue_description = ?, status = ?, assigned_to = ?, service_advisor_id = ? WHERE job_card_id = ?");

                    // MODIFIED: Added 'i' for service_advisor_id
                    // Note: 'assigned_to' is 's' because it's initially raw string, then converted to int or NULL.
                    // If your 'assigned_to' column is strictly INT, you should use 'i' here and ensure $assigned_to is an int or null.
                    // Assuming 'assigned_to' is `assigned_to_mechanic_id` and is an INT.
                    // If assigned_to is an INT, then the bind_param should be "sssiii"
                    // If assigned_to can be NULL, it's better to handle it as an INT and pass NULL.
                    // Let's assume assigned_to and service_advisor_id are integers or NULL.
                    $stmt->bind_param("sssiii", $vehicle_reg, $issue_desc, $status, $assigned_to, $service_advisor_id, $job_card_id);
...
$job_cards = [];
// MODIFIED: Added service_advisor_id and joined with users table for service advisor name
$sql = "SELECT jc.*, u_mech.full_name AS assigned_to_mechanic_name, u_sa.full_name AS service_advisor_name
        FROM job_cards jc
        LEFT JOIN users u_mech ON jc.assigned_to_mechanic_id = u_mech.user_id
        LEFT JOIN users u_sa ON jc.service_advisor_id = u_sa.user_id
...
<body>
            ...
            <table>
                <thead>
                    <tr>
                        ...
                        <th>Status</th>
                        <th>Assigned To</th>
                        <th>Service Advisor</th> <!-- New Column -->
                        <th>Created At</th>
                        <th>Actions</th>
                    </tr>
                </thead>
                <tbody>
                    <?php if (!empty($job_cards)): ?>
                        ...
                        <tr>
                            ...
                            <td><?php echo htmlspecialchars($job_card['vehicle_license']); ?></td>
                            ...
                            <td><?php echo htmlspecialchars($job_card['assigned_to_mechanic_name'] ?? 'N/A'); ?></td>
                            <td><?php echo htmlspecialchars($job_card['service_advisor_name'] ?? 'N/A'); ?></td> <!-- New Cell -->
                        ...
                        <?php endforeach; ?>
                    <?php else: ?>
                        <tr><td colspan="8">No job cards found.</td></tr> <!-- Updated colspan -->
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
    ...
    <div id="editJobCardModal" class="modal">
        <div class="modal-content">
            ...
            <form id="editJobCardForm" method="POST">
                ...
                <!-- New Service Advisor Dropdown -->
                <div class="form-group">
                    <label for="edit-service-advisor">Assign Service Advisor</label>
                    <select id="edit-service-advisor" name="service_advisor_id">
    ...
    <script>
        ...
        function openEditModal(jobCardData) {
            ...
            document.getElementById('edit-vehicle-registration').value = jobCardData.vehicle_license; // Corrected to vehicle_license
            ...
            // New: Set Service Advisor
            const serviceAdvisorSelect = document.getElementById('edit-service-advisor');
            if (jobCardData.service_advisor_id === null || jobCardData.service_advisor_id === undefined) {
                serviceAdvisorSelect.value = '';
            } else {
                serviceAdvisorSelect.value = jobCardData.service_advisor_id;
            }
